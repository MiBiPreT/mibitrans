...
The exact solution of the ADE in Eq.~\eqref{eq:03_wexler} can be simplified into a fully analytical form by splitting the effect of time on the different directions following the principle $C(x,y,z,t)/C_0 = C(x,t)\cdot C(y,x)\cdot C(z,x)/C_0$. Specifically, we apply to Eq.~\eqref{eq:03_wexler} the substitution $\tau = x/v$ (time traveled of advection front) in the error-function terms for the $y$ and $z$ directions. The remaining integral in $\tau$ in Eq.~\eqref{eq:03_wexler} for the transport in $x$-direction can be solved as e.g. shown by \cite{Bear79}. The resulting approximate transport solution is:
...
*Bioscreen* *[Newell et al, 1996, 1997]* makes use of the approximate analytical solution of *Domenico and ...[1987]* in various adaptions. Note that the Domenico-solution has not been derived mathematically rigorous as analytical solution of the ADE, but has been composed of analytical solutions for the individual processes.
