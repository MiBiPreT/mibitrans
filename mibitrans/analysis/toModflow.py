...
class MibitransToModflow:
    """Class that utilizes the Dataclass objects from a Mibitrans model to set up a MODFLOW model (Langevin et al., 2026) 
    by defining parameters and building and running the MODFLOW model.
    ...
    def __init__(
        ...
        Args:
            mt_hydro (mibitrans.data.parameters.HydrologicalParameters) : Dataclass object containing
                hydrological parameters, dispersion and diffusion from HydrologicalParameters.
            mt_att (mibitrans.data.read.AttenuationParameters) : Dataclass object containing adsorption
                and degradation parameters from AttenuationParameters.
            mt_model (mibitrans.data.read.SourceParameters) : Dataclass object containing source parameters
                from SourceParameters.
            mt_source (mibitrans.data.read.ModelParameters) : Dataclass object containing model parameters from
                ModelParameters.
        """
        self.hydro = mt_hydro
        self.att = mt_att
        self.model = mt_model
        self.source = mt_source
    ...
    def _simulation_environment(self):
        """Sets up the simulation environment for MT3DMS.
            
        It initializes paths, executables, and a temporary working directory, which is required to run MT3DMS simulations.
        
        Attributes:
            exe_name_mf (str) : Name of the MODFLOW executable.
            exe_name_mt (str) : Name of the MT3DMS executable.
            workdir (str) : Path to the working directory.
        """
        # Setting the size of figure
        mpl.rcParams["figure.figsize"] = (8, 8)

        # Set up the file names and a directory path
        self.exe_name_mf = "mf2005"
        self.exe_name_mt = "mt3dms"
        
        # Making a new folder to save the model objects in
        self.workdir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "model")
        os.makedirs(self.workdir, exist_ok=True)
        
    def to_modflow(self):
        """Generate MODFLOW and MT3DMS model objects."""
        self._parameters()
        self._simulation_environment()
        self.model_ws = os.path.join(self.workdir)
    ...
    def centerline_modflow(self, label=None):
        ...
        max_x = self.ncol * self.delr
        max_y = np.max(self.source.source_zone_concentration) + 2
