...
![Overview of `mibitrans` package structure and functionalities.](../assets/mibitrans_schematics_structure.svg)
...
In the section **examples**, various example workflows, including application examples, provide inside into the application of `mibitrans`-functionality in in the form of commented Jupyter-Notebooks.
...
This last publication type enables a single-line installation with the `pip` tool, resulting in a very low usage threshold. 
