...
where summation is over all numerical grid cells defined in $x$- and $y$-direction. $V_{cell}=dx \cdot  dy \cdot  Z \cdot  \theta_e$ is the effective pore volume of the cell with $dx$ and $dy$ being the model resolutions in the $x$ and $y$ direction, respectively. 
